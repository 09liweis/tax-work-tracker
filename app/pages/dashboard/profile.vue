...
useHead({
  title: 'My Profile - Tax Work Tracker',
  meta: [
    { name: 'description', content: 'View your personal profile and work records.' },
...
const fetchUserData = async () => {
  if (!loginUser.value?._id) {
    fetchError.value = 'User not logged in'
  ...
  fetchError.value = ''
  try {
    let url = `/api/users/${loginUser.value._id}`
    ...
    if (!res.success) throw new Error(res.error || 'Failed to load profile data')
    userData.value = res
  } catch (err) {
    fetchError.value = err?.message || 'An error occurred while loading profile data'
  } finally {
    isLoading.value = false
  }
}
...
watch(loginUser, async (newValue) => {
  if (newValue) {
    await fetchUserData()
  }
})
