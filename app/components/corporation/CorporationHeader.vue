    ...
    <div class="bg-gradient-to-r from-slate-700 to-slate-800 px-6 py-8">
      <div class="flex items-start justify-between">
        <div class="flex items-center space-x-4">
          <div class="h-20 w-20 rounded bg-white/10 backdrop-blur-sm flex items-center justify-center border border-white/20">
            <span class="text-3xl font-bold text-white">{{ corp.name.charAt(0) }}</span>
          </div>
          <div>
            <h1 class="text-3xl font-bold text-white mb-1">{{ corp.name }}</h1>
        ...
        <div class="flex items-center gap-3">
          <button
            @click="emit('edit')"
            class="flex items-center px-3 py-2 bg-white/10 hover:bg-white/20 text-white rounded transition-colors border border-white/20 backdrop-blur-sm"
          >
